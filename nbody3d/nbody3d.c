...
int main(void){
  ...
  double t, m[N],y[6*N], h, tmax, xmax, ymax, rmax, R, theta,phi;
...
double f(int i, double t, double *y, double *m){
  ...
  if( i <= 3*N-1){
    ...
    return g(i-3*N, y, m, 0);
  }else if( i <= 5*N-1){
    return g(i-4*N, y, m, 1);
  }else if( i <= 6*N-1){
    return g(i-5*N, y, m, 2);
  }
}
...
double g(int k, double *y, double *m, int p){
  ...
  for(j=0; j<N; j++){
    if(j != k){
      r2 = pow(y[j]-y[k], 2.0) + pow(y[N+j]-y[N+k], 2.0) + pow(y[2*N+j]-y[2*N+k], 2.0);
      //printf("r2: %12.4e\n",r2);
